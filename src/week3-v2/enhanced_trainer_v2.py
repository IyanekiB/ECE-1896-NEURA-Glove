...
Usage:
    ...
    python enhanced_trainer_v2.py --dataset data/training_dataset.json --epochs 50
    
    # Fine-tune for new user (calibration)
    python enhanced_trainer_v2.py --calibrate --base-model models/base_model.pth \\
...
class HandPoseDataset(Dataset):
    ...
    def __init__(self, samples: List[dict], pose_to_idx: Dict[str, int], 
        ...
        self.targets = []
        self.pose_labels = []
        self._output_size = None  # Will be calculated from first sample
        ...
        for (pose_name, dataset_num), group_samples in pose_dataset_groups.items():
            ...
            for i in range(num_sequences):
                ...
                # 1. Old format: list of dicts with 'position' and 'rotation'
                # 2. New nested format: dict with finger names -> dict of joint angles
                # 3. Flat dict format: dict of joint names to angle values
                if isinstance(joints_data, list):
                    # Old format: [{'position': [...], 'rotation': [...]}, ...]
                    for joint in joints_data:
                        target.extend(joint['position'])
                        target.extend(joint['rotation'])
                elif isinstance(joints_data, dict):
                    # Check if values are dicts (nested) or scalars (flat)
                    first_value = next(iter(joints_data.values()))
                    if isinstance(first_value, dict):
                        # New nested format: {'thumb': {'metacarpal': 0.5, ...}, 'index': {...}, ...}
                        ...
                        for finger_name in ['wrist', 'thumb', 'index', 'middle', 'ring', 'pinky']:
                            if finger_name in joints_data:
                                ...
                                if isinstance(finger_data, dict):
                                    # Add joint angles in order
                                    for joint_name in sorted(finger_data.keys()):
                                        target.append(float(finger_data[joint_name]))
                                else:
                                    # Single value (e.g., wrist)
                                    target.append(float(finger_data))
                    else:
                        # Flat dict format: {'Thumb_Metacarpal': 0.5, 'Thumb_Proximal': 1.2, ...}
                        # target = [float(v) for v in joints_data.values()]
                        target = []
                        for joint_name, coords in joints_data.items():
                            if isinstance(coords, dict):  # typical structure: {'x': ..., 'y': ..., 'z': ...}
                ...
                # Set output size from first target
...
class EnhancedTrainer:
    ...
    def __init__(self, config: TrainingConfig, pose_names: List[str], 
        ...
        self.model = None  # Will be created after loading dataset
        
        # Loss functions
        self.joint_criterion = nn.MSELoss()
        self.pose_criterion = nn.CrossEntropyLoss()
        
        # Optimizer (will be created with model)
        self.optimizer = None
        self.scheduler = None
        self.calibration_lr = config.CALIBRATION_LEARNING_RATE if calibration_mode else config.LEARNING_RATE
...
def main():
    ...
    # Load dataset (this creates the model with correct output size)
